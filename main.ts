...
scene.onHitWall(SpriteKind.Projectile, function on_hit_wall(bomb2: Sprite, location: tiles.Location) {
    
    bomb2.destroy(effects.disintegrate, 100)
    if (probabilidad_bomba > 10) {
        probabilidad_bomba += 0 - 10
    }
    
})
sprites.onOverlap(SpriteKind.Cursor, SpriteKind.IconoNivel, function on_on_overlap2(sprite: Sprite, otherSprite: Sprite) {
...
sprites.onOverlap(SpriteKind.Player, SpriteKind.Meta, function on_on_overlap3(sprite2: Sprite, otherSprite2: Sprite) {
...
function iniciar_nivel_1() {
    ...
    nena.setStayInScreen(true)
    scene.cameraFollowSprite(nena)
    lista_minas2 = tiles.getTilesByType(assets.tile`
        interrogacion
        `)
    i = 0
...
sprites.onOverlap(SpriteKind.Player, SpriteKind.Projectile, function on_on_overlap4(player2: Sprite, bomb: Sprite) {
...
function menu_inicial() {
    ...
    info.showScore(false)
    music.stopAllSounds()
    music.play(music.stringPlayable("E D C D E E E - ", 120), music.PlaybackMode.LoopingInBackground)
    if (assets.image`
        escapefromusa
        `) {
        scene.setBackgroundImage(assets.image`
...
sprites.onOverlap(SpriteKind.Player, SpriteKind.Enemy, function on_on_overlap5(sprite22: Sprite, otherSprite22: Sprite) {
    game_over_personalizado()
})
function cinematica_lore() {
    ...
    game.showLongText(`
            En un movimiento que nadie vio venir —principalmente
...
let lista_cursores2 : Sprite[] = []
let bot_mirando_derecha = false
let velocidad3 = 0
let distancia3 = 0
let l = 0
let mySpriteBarco2 : Sprite = null
let ola3 : Sprite = null
let ola2 : Sprite = null
let ola1 : Sprite = null
let img_ola : Image = null
let t = 0
let partes_toldo2 : Image[] = []
let i = 0
let lista_minas2 : tiles.Location[] = []
...
let meta_avion : Sprite = null
let img_heli : Image = null
let img_avion : Image = null
let nivel_desbloqueado = 0
let probabilidad_bomba = 0
let lista_minas : number[] = []
let partes_toldo : number[] = []
let lista_cursores : number[] = []
let bot : Sprite = null
...
game.onUpdate(function on_on_update2() {
    ...
    lista_cursores2 = sprites.allOfKind(SpriteKind.Cursor)
